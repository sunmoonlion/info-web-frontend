...
- **镜像名称**：`info-web-frontend:1.0.0`（可在 build.conf 中配置）
...
cd /home/zym/info-web-frontend/mybuild
./build-image.sh

# 2. 部署服务（在 K8s 部署目录）
cd /home/zym/k8s/sunmoonai/app-platform/business-apps/info-app/info-web-frontend/deploy-info-frontend
./deploy-info-frontend.sh deploy dev
```
...
cd /home/zym/info-web-frontend

# 使用 docker
docker build -f mybuild/Dockerfile -t info-web-frontend:1.0.0 .

# 或使用 nerdctl
sudo nerdctl build -f mybuild/Dockerfile -t info-web-frontend:1.0.0 .
```
...
- `INFO_SSR_IMAGE`: 镜像名称（默认: `info-web-frontend`）
- `INFO_SSR_TAG`: 镜像标签（默认: `1.0.0`）
- `INFO_SSR_IMAGE_REGISTRY`: 镜像仓库地址（默认: `harbor.sunmoonai.com:30443`）
- `INFO_SSR_IMAGE_PROJECT`: 镜像项目名（默认: `k8s-images`）
...
5. **镜像分离**：镜像构建和部署已完全分离，构建使用 `build-image.sh`，部署使用 `deploy-info-frontend.sh`
