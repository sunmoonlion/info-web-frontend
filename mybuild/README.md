...
- **镜像名称**：`investment-app-ssr:1.0.0`（可在 build.conf 中配置）
...
cd /home/zym/investment-app-ssr/mybuild
./build-image.sh

# 2. 部署服务（在 K8s 部署目录）
cd /home/zym/k8s/sunmoonai/app-platform/business-apps/investment-app/investment-app-ssr/deploy-investment-ssr
./deploy-investment-ssr.sh deploy dev
```
...
cd /home/zym/investment-app-ssr

# 使用 docker
docker build -f mybuild/Dockerfile -t investment-app-ssr:1.0.0 .

# 或使用 nerdctl
sudo nerdctl build -f mybuild/Dockerfile -t investment-app-ssr:1.0.0 .
```
...
- `INVESTMENT_SSR_IMAGE`: 镜像名称（默认: `investment-app-ssr`）
...
5. **镜像分离**：镜像构建和部署已完全分离，构建使用 `build-image.sh`，部署使用 `deploy-investment-ssr.sh`
