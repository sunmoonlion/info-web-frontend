...
INVESTMENT_SSR_IMAGE="${INVESTMENT_SSR_IMAGE:-investment-app-ssr}"
INVESTMENT_SSR_TAG="${INVESTMENT_SSR_TAG:-1.0.0}"

# 镜像仓库配置（从 build.conf 读取）
INVESTMENT_SSR_IMAGE_REGISTRY="${INVESTMENT_SSR_IMAGE_REGISTRY:-harbor.sunmoonai.com:30443}"
INVESTMENT_SSR_IMAGE_PROJECT="${INVESTMENT_SSR_IMAGE_PROJECT:-k8s-images}"
...
if [[ "$1" == "--tag" && -n "$2" ]]; then
    CUSTOM_TAG="$2"
    INVESTMENT_SSR_TAG="$CUSTOM_TAG"
    log_info "使用自定义标签: $INVESTMENT_SSR_TAG"
fi
...
build_image() {
    log_info "开始构建镜像..."
    log_info "镜像名称: ${INVESTMENT_SSR_IMAGE}:${INVESTMENT_SSR_TAG}"
    ...
    $RUNTIME_CMD build -f "$SCRIPT_DIR/$DOCKERFILE" \
        -t "${INVESTMENT_SSR_IMAGE}:${INVESTMENT_SSR_TAG}" \
        .
    
    if [ $? -eq 0 ]; then
        log_success "镜像构建完成: ${INVESTMENT_SSR_IMAGE}:${INVESTMENT_SSR_TAG}"
        echo ""
        log_info "镜像信息:"
        $RUNTIME_CMD images | grep "${INVESTMENT_SSR_IMAGE}" | grep "${INVESTMENT_SSR_TAG}" | head -1
...
push_image() {
    ...
    # 构建完整镜像名称
    INVESTMENT_SSR_FULL_IMAGE_NAME="${INVESTMENT_SSR_IMAGE_REGISTRY}/${INVESTMENT_SSR_IMAGE_PROJECT}/${INVESTMENT_SSR_IMAGE}:${INVESTMENT_SSR_TAG}"
    
    log_info "完整镜像名称: $INVESTMENT_SSR_FULL_IMAGE_NAME"
    
    # 检查本地镜像是否存在
    if ! $RUNTIME_CMD images | grep -q "${INVESTMENT_SSR_IMAGE}.*${INVESTMENT_SSR_TAG}"; then
        log_error "本地镜像不存在: ${INVESTMENT_SSR_IMAGE}:${INVESTMENT_SSR_TAG}"
        log_info "请先构建镜像: ./build-image.sh"
        exit 1
    fi
    
    # 标记镜像
    $RUNTIME_CMD tag "${INVESTMENT_SSR_IMAGE}:${INVESTMENT_SSR_TAG}" "$INVESTMENT_SSR_FULL_IMAGE_NAME"
    
    # 推送镜像
    if $RUNTIME_CMD push "$INVESTMENT_SSR_FULL_IMAGE_NAME"; then
        log_success "✅ 镜像推送成功: $INVESTMENT_SSR_FULL_IMAGE_NAME"
    else
        log_error "❌ 镜像推送失败: $INVESTMENT_SSR_FULL_IMAGE_NAME"
        log_info "请检查："
        log_info "  1. 镜像仓库配置是否正确"
        if [[ "$RUNTIME_CMD" == "docker" ]]; then
            log_info "  2. 是否已登录镜像仓库（docker login ${INVESTMENT_SSR_IMAGE_REGISTRY}）"
        else
            log_info "  2. 是否已登录镜像仓库（sudo nerdctl login ${INVESTMENT_SSR_IMAGE_REGISTRY}）"
        fi
        log_info "  3. 网络连接是否正常"
        exit 1
    fi
...
# 主函数
main() {
    log_info "Investment App SSR 镜像构建脚本启动"
